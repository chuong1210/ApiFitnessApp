...
namespace Domain.Entities
{
    public class WorkoutPlanItem : AuditableEntity
    {
        ...
        public static WorkoutPlanItem Create(int planId, int workoutId, int itemOrder, int? sets, int? reps, int? durationSeconds, int? restBetweenSetsSeconds)
        {
        ...
        public void UpdateDetails(int itemOrder, int? sets, int? reps, int? durationSeconds, int? restBetweenSetsSeconds)
        {
            if (itemOrder < 0) throw new ArgumentOutOfRangeException(nameof(itemOrder), "Item order must be non-negative.");
            ItemOrder = itemOrder;
            Sets = sets > 0 ? sets : null;
